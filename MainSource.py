...
import MainInterfaces

servers = []
logs = []


class info_sender(MainInterfaces.interface_info_sender):
    def send(self, data):
        print(data)
        return


class server_log_repository(MainInterfaces.interface_server_log_repository):
    def _check(self, ip):
        for log in logs:
            if log['ip'] == ip:
...
class server_repository(MainInterfaces.interface_server_repository):
...
class addserver_command(MainInterfaces.executor):
    def execute(self, query):
        self.repos.add_server(query['ip'])


class removeserver_command(MainInterfaces.executor):
    def execute(self, query):
        self.repos.remove_server(query['ip'])


class server_information_collector_command(MainInterfaces.executor):  # взять инфу с сервера и отправить в отправитель
    def execute(self, query):
...
class log_collector_command(MainInterfaces.executor):
    def execute(self, query):
        isender = info_sender()
        isender.send(logs)


class request_receiver(MainInterfaces.interface_request_receiver):
